...
$\rho$ is the test sensitivity, $\varphi$ is the probability of a patient being 
positive on admission and $\beta$ is the transmission rate. The likelihood
of the colonisation times given the parameters of the transmission model
is given by
...
of patient $i$ becoming colonised on day $j$ which is defined below. The parameters 
$b$ and $s$ and the matrix $\Sigma$ are introduced to account for the effect of
antibiotics and are discussed in more detail below. Inference is 
conducted via MCMC, where each step consists of updating the parameters of the transmission 
model using the current colonisation times and then updating the colonisation 
...
We also introduce a multiplicative effect of antibiotics on transmissibility and 
susceptibility and denote the probability of patient i becoming colonised on day j as:
...
```
We can also plot the chains and densities of individual parameters:
