#' Run data augmented MCMC with discrete typing data.
#'
#' @param data a list containing 4 elements:
#' \describe{
#'   \item{patients}{a dataframe with three columns. Each row
#'   corresponds to a patient, the columns are admission, type, first_positive_test
#'   and discharge. Entries are in days. If there is no type for a patient, type
#'   equals -1. If there is no positive test for a patient,
...
#'}
#' @export
run_mcmc_with_types <- function(data = NULL, priors = NULL, configuration = NULL){
  ...
  n_types <- length(unique(patients$type)) - 1
  ...
  patients$status <- "s"
  # source: -1 = not infected, 0 = environment, pt = source
  patients$source <- -1
  patients$first_onward_transmission <- 20000
  ...
  if (length(index) > 0){
    ...
    patients[index,]$source <- 0
  }

  #set up aquisitions: initially as any infected after the first day
  index <- which(patients$first_positive_test != 20000 & patients$first_positive_test > patients$admission)
  if (length(index) > 0){
    for (pt in index){
      patients[pt, ]$colonisation <- patients[pt,]$first_positive_test
      patients[pt, ]$status <- "a"


      #get vector of possible sources
      col_date <- patients[pt, ]$colonisation
      col_type <- patients[pt, ]$type
      possible_sources <- which(patients$admission <= col_date &
                                patients$discharge >= col_date &
                                patients$colonisation < col_date &
                                patients$type == col_type)

      possible_sources <- possible_sources[possible_sources != pt]

      #no source: set as importation
      if (length(possible_sources) == 0){
        patients[pt, ]$colonisation <- patients[pt, ]$admission - 1
        patients[pt, ]$status <- "p"
        patients[pt, ]$source <- 0
      }
      #sources available: sample a source
      if (length(possible_sources) == 1){
        new_source <- possible_sources[1]
        patients[pt, ]$source <- new_source
        new_onward_transmission <- update_onward_transmission(new_source,
                                                              patients$source,
                                                              patients$colonisation,
                                                              n_patients)
        patients[new_source, ]$first_onward_transmission <- new_onward_transmission
  ...
  # sources
  output_matrix_sources <- matrix(0, nrow = n_iter, ncol = n_patients)
  ...
  for (iter in 1:n_iter){
    ...
    for (k in 1:N_local_status_updates){
      ...
      if (type_of_move_sampling == 1){
        ...
        source_cur <- patients$source
        type_cur <- patients$type
        onward_cur <- patients$first_onward_transmission

        col_cand <- col_cur
        status_cand <- status_cur
        source_cand <- source_cur
        type_cand <- type_cur
        onward_cand <- onward_cur

        log_hastings_ratio <- NA

        # define window when patient could have aquired. The last possible day
        # of colonisation is the minimum of the discharge, first positive test
        # and first onward transmission.


        first_possible_acquisition_day <- patients[pt, "admission"]

        last_possible_acquisition_day <- min(patients[pt, ]$discharge,
                                             patients[pt, ]$first_positive_test,
                                             patients[pt, ]$first_onward_transmission -1)

        dt <- last_possible_acquisition_day - first_possible_acquisition_day + 1

        #propose an importation, with probabilty w
        if (runif(1) < w){
          if(status_cur[pt] == "p") { # import --> import
            # no change
          } else { # a --> p
            col_cand[pt] <- patients[pt,]$admission - 1
            status_cand[pt] = "p"

            #in the candidate state, source is the environment (0) and the first onward
            #transmission of the current source changes
            source_cand[pt] <- 0
            onward_cand[source_cur[pt]] <- update_onward_transmission(source_cur[pt],
                                                                      source_cand,
                                                                      col_cand,
                                                                      n_patients)
          }
          ...
          #sample a new source: here, only sample from patients with same
          #type as the patient already has a type

          col_date <- col_cand[pt]
          col_type <- type_cand[pt]
          possible_sources <- which(patients$admission <= col_date &
                                      patients$discharge >= col_date &
                                      patients$colonisation < col_date &
                                      patients$type == col_type)
          ...
          #no source: reject move
          if (length(possible_sources) == 0){
            col_cand[pt] <- col_cur[pt]
            status_cand[pt] <- status_cur[pt]
          }

          #sources available: sample a source
          if (length(possible_sources) == 1){
            new_source <- possible_sources[1]
            source_cand[pt] <- new_source

            #update onward transmission of candidate source
            new_onward_transmission <- update_onward_transmission(new_source,
                                                                  source_cand,
                                                                  col_cand,
                                                                  n_patients)

            onward_cand[new_source] <- new_onward_transmission

            #update onward transmission of current source (if applicable)
            if (status_cur[pt] == "a"){
              new_onward_transmission <- update_onward_transmission(source_cur[pt],
                                                                    source_cand,
                                                                    col_cand,
                                                                    n_patients)
              onward_cand[source_cur[pt]] <- new_onward_transmission
            }
          }
        ...
        #compute hastings ratios

        n_sources_cand = length(possible_sources)

        col_date <- col_cur[pt]
        col_type <- type_cur[pt]
        possible_sources_cur <- which(patients$admission <= col_date &
                                    patients$discharge >= col_date &
                                    patients$colonisation < col_date &
                                    patients$type == col_type)

        possible_sources_cur <- possible_sources_cur[possible_sources_cur != pt]

        n_sources_cur = length(possible_sources_cur)
        ...
        if (status_cur[pt] == "p"){
          ...
          if (status_cand[pt] == "p"){
            # a -> p
            log_hastings_ratio = log(( 1 - w ) / ( dt * n_sources_cur * w ))
          } else if (status_cand[pt] == "a"){
            # a -> a
            log_hastings_ratio = log(n_sources_cand / n_sources_cur)
          }
        }



        ll_cur <- log_lik_overall_with_types(patients$admission,
                                  patients$discharge,
                                  patients$colonisation,
                                  patients$source,
                                  patients$status,
        ...
        ll_cand <- log_lik_overall_with_types(patients$admission,
                                              patients$discharge,
                                              col_cand,
                                              source_cand,
                                              status_cand,
        ...
        if (log(runif(1, 0, 1)) < log_proposal_ratio){
          ...
          #update current values with proposed
          patients$colonisation[pt] = col_cand[pt]
          patients$status[pt] = status_cand[pt]
          patients$source[pt] = source_cand[pt]
          patients$first_onward_transmission = onward_cand
        }
      ...
      if (type_of_move_sampling == 2){
        ...
        vector_ap <- get_ap_with_types(patients$status,
        ...
        if (n_s > 0) {
          ...
          if (runif(1) < w) {
            ...
            #sample a new source: here, since the patient has never been tested positive
            #there is no type. So sample any source and inherit type from source
            col_date <- col_cand[pt]
            possible_sources <- which(patients$admission <= col_date &
                                      patients$discharge >= col_date &
                                      patients$colonisation < col_date)
            ...
            if (length(possible_sources) == 1){
              ...
              type_cand[pt] <- type_cur[new_source]
            }
            ...
            log_hastings_ratio = log(n_s * dt * length(possible_sources) / ((n_ap + 1) * (1 - w)))
      ...
      if (type_of_move_sampling == 3){
        ...
        if (n_ap > 0) {
          ...
          # define window when patient could have aquired. The last possible day
          # of colonisation is discharge, because if the patient has an onward
          # transmission it is not elegible for colonisation removal.
          first_possible_acquisition_day <- patients[pt, ]$admission
          last_possible_acquisition_day = patients[pt, ]$discharge
          ...
          if(status_cur[pt] == "p") {
            ...
            log_hastings_ratio = log(n_ap * (1 - w ) / ((n_s + 1) * dt * n_sources_cur))
    ...
    # sources
    output_matrix_sources[iter, ] <- patients$source
  }
  ...
  output <- list(output_dataframe_params, output_dataframe_statuses, output_matrix_sources)
  names(output) <- c("parameters","statuses", "sources")
  return(output)
}
